...
import { StyleSheet, Text, Animated, TouchableOpacity, Image , Dimensions, Share, TextInput, FlatList} from 'react-native'
...
//Custom road coordinates ends here
 ...
 useEffect(() => {
  ...
  const [newBuildingImage,setNewBuildingImage] = useState('none')
  ...
  const handleMarkerPressDB = async (id) => {
    setPopInfo([])
   ...
   setDataPopInfo(newData)
      ...
      try {
        const res = await axios.get(`https://backendnorsumaps.onrender.com/filesBuilgindImg/${newData[0].buildingNewImage}`, {
        responseType: 'arraybuffer',
      })

      const base64 = Buffer.from(res.data, 'binary').toString('base64');
      setNewBuildingImage(`data:image/jpeg;base64,${base64}`);
      } catch (err) {
...
const [searchQuery, setSearchQuery] = useState('');
  const [filteredMarkers, setFilteredMarkers] = useState(newBuildingData);
  ...
  const handleSearch = (query) => {
    ...
    if(newBuildingData.length > 0){
      if (query) {
        const filtered = newBuildingData.filter(marker => marker.title.includes(query));
        ...
        setFilteredMarkers(newBuildingData);
      }
    }
  };

  const handleMarkerPress2 = (coordinate) => {
    console.log(coordinate)
    mapViewRef.current.animateToRegion({
      ...coordinate,
      latitudeDelta: 0.01,
  ...
  const showSearch = () => {
     setOpenSearch(item => !item)
  }

  return (
    <View style={styles.container}>
      <TouchableOpacity style = {styles.sidebarBtn} onPress={toggleSidebar}>
           <Image style ={styles.sidebarIcon}  source={icon.Menu} alt='1'/>
       </TouchableOpacity>
     <Animated.View style={[styles.sidebar, {left: sidebarWidth}]}>
...

      {
       ...
       <TouchableOpacity style = {styles.serachBtn} onPress={() => {showSearch()}}>
           <Text style = {{color: 'white'}}>Search</Text>
       </TouchableOpacity>
              
       {
          openSearch ? 
          <View style={{ position: 'absolute', top: 70, left: 10, right: 10, zIndex: 5,borderRadius: 10 }}>
          <TextInput
          ...
          {searchQuery.length > 0 && (
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => handleMarkerPress2({latitude: item.latitude, longitude: item.longitude})}>
                  <Text style={{ padding: 10, backgroundColor: 'white' }}>{item.title}</Text>
                </TouchableOpacity>
              )}
            />
          )}
      ...
      
     {
        ...
        <View style = {styles.modalImageContainer}>

          {
             ...
             <Image style = {styles.modalMainImage} source={background.CASBuilding2} alt='cba'/>
             : 
             <Image style = {styles.modalMainImage} source={{uri: newBuildingImage}} alt='cba'/>  
